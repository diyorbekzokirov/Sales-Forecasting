# This is the Python code that predicts sales better since it uses 21 different models and ensembles them
...
try:
    ...
    # We're not in a Jupyter notebook, so use a different matplotlib backend
    import matplotlib
...
def assign_dates(data, flag, dates = ''):
    if flag == 'validation':
        ...
        dates.time = pd.to_datetime(dates.time, infer_datetime_format=True)
        ...
        last_date = pd.to_datetime(dates.time[0], infer_datetime_format=True)
        print(last_date)
...
#%%
os.chdir(r'/home/unsmartboy/Documents/retail')
